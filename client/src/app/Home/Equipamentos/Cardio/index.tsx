...
import { useGetCardioEquipmentQuery } from "@/state/api";

const Cardio = () => {
  const { data: cardioMachines } = useGetCardioEquipmentQuery();
  const [searchQuery, setSearchQuery] = useState("");
  console.log(cardioMachines);
  // Filter machines based on search query
  const filteredMachines =
    cardioMachines?.filter((machine) =>
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredMachines.map((machine) => (
            <div
              key={machine.id}
              className="dark:bg-gray-900 bg-slate-200 rounded-lg overflow-hidden border border-gray-800 transform hover:scale-105 transition-transform duration-300"
            >
              {/* Image */}
